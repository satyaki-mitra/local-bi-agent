...
| DeepSeek-R1 model | 8b recommended | `ollama pull deepseek-r1:8b` |
...
Run `ollama pull deepseek-r1:8b` (local) or `docker exec localgenbi-ollama ollama pull deepseek-r1:8b` (Docker).
...
**Slow first response**
The first query after startup loads DeepSeek-R1 into memory. On CPU-only hardware this can take 30–120 seconds. Subsequent queries are faster. Monitor backend logs to confirm inference is progressing.
